...
While `code-intelligence` provides high-precision dead code detection by blending AST analysis, call graphs, and calibrated machine learning, static analysis of dynamic codebases has inherent boundaries. This document details the tool's limitations, edge cases across supported languages, and recommended mitigations.
...
Runtime string lookups and reflection bypass static call graph construction:
...
| **RAM Usage** | 4,096 MB | Configurable via `CI_MEMORY_LIMIT_MB` |
| **File Traversal** | 10,000 files | Exceeding files require a `--max-files` override or incremental caching |
...
|------------|--------|----------|-----------------------|
| **Reflection** | False dead candidates | High | AST literal extraction + manual whitelist |
| **Polymorphism / DI** | Indirect method usage | Medium | Interface and trait methods protected |
| **FFI / WASM Exports** | Zero internal callers | High | Automated attribute export detection |
| **Macros / CodeGen** | Unparsed expansions | Medium | Generated file markers + reduced penalty |
| **IPC & Dynamic Imports** | Decoupled execution | Medium | IPC literal AST matching |
| **Large Graphs (>5k nodes)** | Slower graph traversals | Low | Cycle skipping + disk caching |
