## Document 6: `docs/architecture.md`
...
The pipeline is the heart of the system. It processes a project through stages:
...
┌─────────────────────────────────────────────────────────────────┐
│                   1. is_never_dead()                           │
│  Check if function should never be considered dead:            │
...
│  BFS from roots through call graph:                           │
│  - Reachable → Alive                                          │
...
| CallGraph (100k nodes) | 200-400 MB | Depends on edges |
...
| Cycle Detection | O(F + C) | Skip for > 5000 nodes |

### Scaling

| Size | Functions | Time | Memory |
|------|-----------|------|--------|
| Small | < 1000 | < 5s | < 100 MB |
...
- **File system access**: Read-only except for cache

### LLM Integration

- **API keys**: Stored in config file (local)
...
2. Add language config in `TreeSitterParser`
3. Add language-specific patterns
4. Test with fixtures

### Adding a New Feature

1. Define feature in `feature_schema.rs`
2. Extract feature in `features.rs`
3. Add to `FunctionFeatures` struct
...
5. **More Languages**: C++, C#, Swift, Kotlin
